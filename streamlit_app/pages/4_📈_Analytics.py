...
def render_category_breakdown(df: pd.DataFrame):
    """Render pie chart showing spending breakdown by utility category."""
    if df.empty:
        return

    category_totals = df.groupby("Utility_Category")["Cost_Amount"].sum().reset_index()
    category_totals = category_totals.sort_values("Cost_Amount", ascending=False)

    fig = go.Figure(
        data=[go.Pie(
            labels=category_totals["Utility_Category"],
            values=category_totals["Cost_Amount"],
            textinfo="label+value+percent",
            marker=dict(line=dict(color="white", width=2))
        )]
    )
    fig.update_layout(
        title="Spending Breakdown by Utility Category",
        height=400
    )
...
def render_month_over_month_comparison(df: pd.DataFrame):
    """Compare current month spending vs previous month by category."""
    if df.empty:
    ...
    for category in sorted(all_categories):
        ...
        comparison_data.append({
            "Category": category,
            f"{months[-2]}": f"${prev:.2f}",
            f"{months[-1]}": f"${curr:.2f}",
            "Change": f"{status} ${abs(change):.2f}",
