...
st.set_page_config(
    page_title="Landlord Portal - CoHabitant",
    page_icon="🏠",
    layout="wide",
)


def check_landlord_authenticated():
    """Enforce landlord-only access to this page."""
    if st.session_state.get("logged_in_user_id") is None:
        st.error("You must be logged in to access the Landlord Portal.")
        st.stop()

    if st.session_state.get("logged_in_role") != "Landlord":
        st.error("Access denied. This page is restricted to Landlords.")
        st.stop()


def table_has_column(table_name: str, column_name: str) -> bool:
    """Return True if the table contains the specified column."""
    sql = """
    SELECT COUNT(*) AS Cnt
...
def load_my_properties(landlord_id: int) -> pd.DataFrame:
    """READ: Load landlord properties joined with lease agreement details."""
    sql = """
    SELECT
        p.Property_ID,
        p.Street_Address,
        ...
        p.Zip_Code,
        p.Max_Occupancy,
        COUNT(la.Lease_ID) AS Lease_Count,
        MIN(la.Start_Date) AS First_Lease_Start,
        MAX(la.End_Date) AS Last_Lease_End
    FROM dbo.PROPERTY p
    LEFT JOIN dbo.LEASE_AGREEMENT la ON p.Property_ID = la.Property_ID
    WHERE p.Landlord_ID = ?
    GROUP BY
        p.Property_ID,
        p.Street_Address,
...
def load_tenants() -> pd.DataFrame:
    """Load tenant options for schemas where LEASE_AGREEMENT requires Tenant_ID."""
    sql = """
    SELECT
        t.Tenant_ID,
...
def load_utility_companies() -> pd.DataFrame:
    """Load utility companies, with fallback to UTILITY_TYPE for schema compatibility."""
    sql_companies = """
    SELECT
        Utility_Company_ID,
        Company_Name
    FROM dbo.UTILITY_COMPANY
    ORDER BY Company_Name
    """
    try:
        df = run_query(sql_companies)
        logger.info("Loaded utility companies from dbo.UTILITY_COMPANY")
        return df
    except Exception as exc:
        logger.error("UTILITY_COMPANY read failed, falling back to UTILITY_TYPE: %s", exc)
        sql_types = """
        SELECT
            Utility_Type_ID AS Utility_Company_ID,
            Type_Name AS Company_Name
        FROM dbo.UTILITY_TYPE
        ORDER BY Type_Name
        """
        df = run_query(sql_types)
        logger.info("Loaded utility companies fallback from dbo.UTILITY_TYPE")
        return df


def tab_my_properties(landlord_id: int):
    """My Properties tab (READ)."""
    ...
    try:
        ...
        logger.info("Loaded landlord properties for user %s", landlord_id)
    except Exception as exc:
    ...
    with st.form("add_property_form", clear_on_submit=True):
        ...
        if submitted:
            if not street.strip() or not city.strip() or not state_province.strip() or not postal_code.strip():
                st.error("Street Address, City, State/Province, and Postal Code are required.")
                return

            try:
                # Requested schema path
                insert_requested = """
                ...
                logger.info("Property created for landlord %s using requested schema columns", landlord_id)
                st.rerun()
            except Exception as exc:
                logger.error("Requested PROPERTY insert failed; trying fallback schema: %s", exc)
                try:
                    # Current project schema fallback
                    insert_fallback = """
                    INSERT INTO dbo.PROPERTY (
                        Landlord_ID,
                        Street_Address,
                    ...
                    st.success("Property created successfully.")
                    logger.info("Property created for landlord %s using fallback schema columns", landlord_id)
                    st.rerun()
                except Exception as fallback_exc:
                    st.error(f"Failed to create property: {fallback_exc}")
                    logger.error("Property insert failed for landlord %s: %s", landlord_id, fallback_exc)


def tab_create_lease(landlord_id: int):
    """Create Lease tab (CREATE)."""
    ...
    try:
        ...
        logger.error("Property load for lease creation failed: %s", exc)
        return
    ...
    if requires_tenant_id:
        try:
            ...
            logger.error("Tenant load failed for lease creation: %s", exc)
    ...
    with st.form("create_lease_form", clear_on_submit=True):
        ...
        if requires_tenant_id:
            if tenant_options:
                ...
                st.warning("No tenants available. Lease creation may fail if Tenant_ID is required.")
        ...
        if submitted:
            ...
            try:
                ...
                logger.info("Lease created for property %s using requested schema columns", property_id)
                st.rerun()
            except Exception as exc:
                logger.error("Requested LEASE_AGREEMENT insert failed; trying fallback schema: %s", exc)
                try:
                    if not requires_tenant_id:
                        raise Exception("Fallback lease path unavailable: Tenant_ID not detected.")
                    if not selected_tenant_label:
                        raise Exception("Tenant selection is required by current database schema.")
                    ...
                    st.success("Lease created successfully.")
                    logger.info(
                        "Lease created for property %s and tenant %s using fallback schema columns",
                        property_id,
                        tenant_id,
                    )
                    st.rerun()
                    ...
                    logger.error("Lease insert failed for property %s: %s", property_id, fallback_exc)


def tab_log_utility_bill(landlord_id: int):
    """Log Utility Bill tab (CREATE)."""
    ...
    try:
        ...
        st.error(f"Could not load properties: {exc}")
        logger.error("Property load for utility logging failed: %s", exc)
        return

    if properties_df.empty:
        st.info("Add at least one property before logging utility bills.")
        return

    try:
        companies_df = load_utility_companies()
    except Exception as exc:
        st.error(f"Could not load utility companies: {exc}")
        logger.error("Utility company/type load failed: %s", exc)
        return

    if companies_df.empty:
        st.info("No utility companies available.")
        return
    ...
    company_options = {
        f"#{int(row['Utility_Company_ID'])} | {row['Company_Name']}": int(row["Utility_Company_ID"])
    ...
    with st.form("log_utility_bill_form", clear_on_submit=True):
        ...
        if submitted:
            if billing_period_end < billing_period_start:
                st.error("Billing Period End cannot be earlier than Billing Period Start.")
                return
            ...
            try:
                # Requested schema path
                insert_requested = """
                INSERT INTO dbo.UTILITY_READING (
                    Property_ID,
                    Utility_Company_ID,
                    Reading_Date,
                    Cost_Amount,
                ...
                st.success("Utility bill logged successfully.")
                logger.info("Utility bill logged for property %s using requested schema columns", property_id)
                st.rerun()
            except Exception as exc:
                logger.error("Requested UTILITY_READING insert failed; trying fallback schema: %s", exc)
                try:
                    # Current project schema fallback
                    insert_fallback = """
                    INSERT INTO dbo.UTILITY_READING (
                    ...
                    st.success("Utility bill logged successfully.")
                    logger.info("Utility bill logged for property %s using fallback schema columns", property_id)
                    st.rerun()
                except Exception as fallback_exc:
                    st.error(f"Failed to log utility bill: {fallback_exc}")
                    logger.error("Utility bill insert failed for property %s: %s", property_id, fallback_exc)


def main():
    """Landlord portal entrypoint."""
    check_landlord_authenticated()
    ...
    with tab1:
        tab_my_properties(landlord_id)

    with tab2:
        tab_add_property(landlord_id)

    with tab3:
        tab_create_lease(landlord_id)

    with tab4:
        tab_log_utility_bill(landlord_id)
